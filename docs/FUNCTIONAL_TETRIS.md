...
### コンパイラ最適化後に実現 (PR #58-#63 + Phase 3/4 リファクタリング)

chip8-lang コンパイラの最適化により、当初「不可能」とされた機能が実現:

| 機能 | 以前の制約 | 解決した最適化 |
|------|-----------|--------------|
| `GameState` struct で状態管理 | 6 フィールドでレジスタ超過 | struct レジスタ保持 + メモリフォールバック (PR #60) |
| 末尾再帰 `game_loop(state)` | struct 引数 + 関数呼び出しで超過 | インライン展開 + レジスタ圧力チェック (PR #63) |
| `\|>` パイプ演算子 | struct がないと活かせない | struct 実現により `state \|> tick \|> game_loop` が可能に |
| match ジャンプテーブル | 線形探索で命令数膨大 | `JpV0 (BNNN)` による O(1) ディスパッチ (PR #63) |

**現在のコード構造**:
```
game_loop(state) = state |> tick |> game_loop          -- パイプ + TCO
handle_fall(state) = ... state |> land_and_spawn       -- パイプで着地処理
handle_input(state) = if key4 ... else if key6 ...     -- 単一関数に統合
```
...
| 理想形 | 制約 | 現実的な代替 |
|--------|------|-------------|
| `Pos` struct を関数引数に | `try_move` の 6 引数 + struct でレジスタ超過 | スカラー `u8` 引数 (px, py) |
| `Input` enum + match で入力分岐 | enum 生成 + match 分岐でレジスタ圧迫 | `if is_key_pressed(N)` の直接呼び出し |
| 純粋な衝突判定 (描画なし) | CHIP-8 にフレームバッファ読み取り API がない | XOR 描画 + VF フラグで判定 (フリッカーの原因) |

### 結論

コンパイラ最適化により、CHIP-8 の 15 レジスタ制約の中でも **「不変 struct + パイプ演算子 + 末尾再帰」** のスタイルが実用的に動作することが証明された。残る課題は XOR 描画ベースの衝突判定によるフリッカーで、これは CHIP-8 のハードウェア制約 (フレームバッファ読み取り不可) に起因する。
